...
import dataclasses
import networkx as nx   #to use FR

from pulser import InterpolatedWaveform, Pulse, Sequence, Register
from pulser_myqlm import IsingAQPU

#THIS IS THE BENCH.PY VARIANT TO TRY TO SCALE UP TO 11 QUBITS AND MORE. THE IDEA IS TO USE FRUCHTERMAN-REINGOLD (FR)
#TO INITIALIZE THE PROBLEM, TO FIND POPULATION AT TIME 0 TO WICH APPLY THE GA.
...
OUTPUT_CSV = f".././new_csv/experiment_registry_10x10_FR_init.csv"
...
# ==========================================
# FRUCHTERMAN-REINGOLD LAYOUT GENERATOR
# ==========================================
def get_fr_coordinates(Q, max_radius=20.0):
    """
    Computes spatial coordinates using the Fruchterman-Reingold force-directed algorithm.
    This acts as a physics-agnostic topological unwrapper.
    """
    # Convert the QUBO matrix into a NetworkX graph (using absolute weights)
    G = nx.from_numpy_array(np.abs(Q))
    
    # Remove self-loops (diagonal elements) as they interfere with the layout
    G.remove_edges_from(nx.selfloop_edges(G))
    
    # Execute Fruchterman-Reingold (attractive springs and repulsive electrical forces)
    # scale: expands coordinates within our allowed hardware radius
    pos = nx.spring_layout(G, k=1.5, scale=max_radius, iterations=200, seed=42)
    
    # Extract coordinates into a (N_ATOMS, 2) numpy array
    coords = np.array([pos[i] for i in range(len(Q))])
...
def optimize_embedding(Q, num_restarts=10):
    ...
    def improved_topological_mae_fitness_func(ga_instance, solution, solution_idx):
        ...
        # Hard spatial constraints (Unchanged from original implementation)
        if np.any(distances < MIN_DIST):
            penalty += np.sum(np.clip(MIN_DIST - distances, 0, None)) * 100000.0 
        ...
        # DIFFERENCE 1: Contain the 1/r^6 explosion.
        # Previous issue: If two atoms got too close, V skyrocketed to infinity,
        # destroying the gradient for the rest of the array.
        # Solution: Clip the maximum physical potential to 5x the maximum target Q value.
        max_q = np.max(np.abs(Q_triu)) if np.max(np.abs(Q_triu)) > 0 else 1.0
        V_triu_clipped = np.clip(V_triu, 0, max_q * 5.0)

        # DIFFERENCE 2: Removal of 'bond_importance'.
        # Previous issue: Multiplying the error by Q_triu masked crosstalk. 
        # If Q was 0 (no desired connection), the calculated error was 0 even if physical V was huge.
        # Solution: Use a boolean mask to separate desired bonds from unwanted ones.
        active_bonds = np.abs(Q_triu) > 1e-5
        
        # Error calculation 1: Deviation from target connections (where Q > 0)
        error_active = np.sum(np.abs(V_triu_clipped[active_bonds] - Q_triu[active_bonds]))
        
        # Error calculation 2: Crosstalk penalty (where Q = 0)
        # Sum all the physical potential generated by mistake between independent nodes.
        # The * 2.0 multiplier forces the GA to prioritize distancing unconnected nodes.
    ...
    print("  -> Generating Smart Initial Population (Fruchterman-Reingold Seeding)...")
    
    # 1. Generate the base topological layout using FR
    # We use 0.8 to keep a safety margin from the laser grid boundaries
    fr_base_coords = get_fr_coordinates(Q, max_radius=MAX_RADIUS * 0.8)
    base_chromosome = fr_base_coords.flatten()
    
    # 2. Initialize the smart population
    initial_pop = []
    
    # Add the pristine FR layout as the first individual
    initial_pop.append(base_chromosome)
    
    # Generate 399 mutants by adding small spatial noise to the FR layout
    # This allows the GA to explore the immediate vicinity of the unwrapped topology
    for _ in range(399):
        noise = np.random.uniform(-2.0, 2.0, size=len(base_chromosome))
        mutated_chromosome = np.clip(base_chromosome + noise, -MAX_RADIUS, MAX_RADIUS)
        initial_pop.append(mutated_chromosome)

    print(f"  -> Smart Population Ready. Starting Multi-Start ({num_restarts} runs)...")
    
    for run_idx in range(num_restarts):
        ga = pygad.GA(
            num_generations=1000,            # Reduced from 2000 due to smart initialization
            num_parents_mating=20,
            fitness_func=improved_topological_mae_fitness_func,
            initial_population=initial_pop,  # Injecting the seeded population
            # sol_per_pop=400,               # MUST be commented out when using initial_population
            num_genes=N_ATOMS * 2,
            ...
            mutation_probability=[0.10, 0.02],
