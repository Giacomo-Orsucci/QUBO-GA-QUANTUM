...
# Ensure this path is exactly the same as the one used in bench.py
OUTPUT_CSV = f"./new_csv/experiment_registry_SA_FR_{instance_name}.csv"

# =====================================================================
# --- NEW LOGIC: READ JOB_ID AND FITNESS DIRECTLY FROM CSV ---
# =====================================================================
try:
    df_registry = pd.read_csv(OUTPUT_CSV)
    # Fetches the very last row (the most recent execution)
    last_run = df_registry.iloc[-1]
    
    JOB_ID = str(last_run["Juelich_Job_ID"])
    FITNESS_TYPE = str(last_run["Fitness_Metric"])
    N_ATOMS = int(last_run["N_Nodes"])
    
    print(f"[INFO] Auto-loaded from CSV -> Job ID: {JOB_ID} | Fitness: {FITNESS_TYPE}")
except FileNotFoundError:
    print(f"[ERROR] Cannot find the CSV file: {OUTPUT_CSV}")
    print("Make sure you ran bench.py first to generate the file.")
    exit()
except Exception as e:
    print(f"[ERROR] Error while reading JOB_ID and FITNESS_TYPE from the CSV. Details: {e}")
    exit()
# =====================================================================
...
# 1. Prepare the quantum data package to save (Added explicit string formatting)
quantum_data = {
    "Top_State_QPU": f"'{top_state_bitstring}'" if top_state_bitstring else "None",
